...
$(document).ready(function () {
    ...
    $('#search input[name=\'search\']').parent().find('button').on('click', function() {
        ...
        // workdo Search Start 
        var category_id = $('#workdo-search-category').val();
        if (category_id > 0) {
...
// workdo Search Start 
    $(".workdo-search").keyup(function(){
        if(($(this).val()).length >= 3) {
            var category_id = $('#workdo-search-category').val();
            console.log(category_id);
            $.ajax({
                url: 'index.php?route=common/workdo_search|search',
                type: 'post',
                data : 'search_string=' + $(this).val() + '&category_id=' + category_id,
                beforeSend  : function () {
                    $('.workdo-search-loader').show();
                },
                success: function(result){
                    console.log(result);
                    $('.workdo-search-result').html(result);
                    $('.workdo-search-loader').hide();
                }
            });
            //console.log(this);
        }
        else {
            $('.workdo-search-result').html('');
        }
    });
    // workdo Search END
    // Tooltip
...
$(document).ready(function () {
    ...
    $('#button-grid').on('click', function () {
        ...
        $('#product-list').attr('class', 'row product-grid row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-3');
    ...
    if (localStorage.getItem('display') == 'list') {
        ...
        $('#product-list').attr('class', 'row product-grid  row-cols-1 row-cols-sm-3 row-cols-md-3 row-cols-lg-3');
        $('#button-grid').addClass('active');
    }
